...
A [timoni.sh](http://timoni.sh) module for deploying acme-cluster-issuer to Kubernetes clusters.
...
```shell
timoni -n default apply acme-cluster-issuer oci://ghcr.io/westelh/timoni/modules/acme-cluster-issuer
```
...
```shell
timoni -n default delete acme-cluster-issuer
```
