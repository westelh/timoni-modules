...
//timoni:generate timoni mod vendor crd -f vault-secrets-operator/chart/crds/secrets.hashicorp.com_hcpvaultsecretsapps.yaml

package v1beta1

import "strings"

// HCPVaultSecretsApp is the Schema for the hcpvaultsecretsapps
// API
#HCPVaultSecretsApp: {
	...
	// HCPVaultSecretsAppSpec defines the desired state of
	// HCPVaultSecretsApp
...
#HCPVaultSecretsAppSpec: {
	...
	// HCPAuthRef to the HCPAuth resource, can be prefixed with a
	// namespace, eg:
	// `namespaceA/vaultAuthRefB`. If no namespace prefix is provided
	// it will default
	// to the namespace of the HCPAuth CR. If no value is specified
	// for HCPAuthRef the
	// Operator will default to the `default` HCPAuth, configured in
	// the operator's
	// namespace.
	hcpAuthRef?: string
	...
	// application(s)
	// consuming the HCP Vault Secrets App does not support
	// dynamically reloading a
	// rotated secret. In that case one, or more
	// RolloutRestartTarget(s) can be
	// configured here. The Operator will trigger a "rollout-restart"
	// for each target
	// whenever the Vault secret changes between reconciliation
	// events. See
	...
	syncConfig?: {
		dynamic?: {
			// RenewalPercent is the percent out of 100 of a dynamic secret's
			// TTL when
			// new secrets are generated. Defaults to 67 percent plus up to
			// 10% jitter.
